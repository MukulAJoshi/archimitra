...
(defn print-usage [{:keys [biff/db]}]
  ...
  (let [n-users (nth (q db
    ...
    (log/info "There are" n-users "users."
              ...
              "by setting `:org.archimitra/enable-worker false` in config.edn)")))

(defn alert-new-user [{:keys [org.archimitra/enable-worker biff.xtdb/node]} tx]
  (doseq [_ [nil]
          :when enable-worker
